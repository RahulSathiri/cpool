...
/*
			String query="select  cpl.consultant_id,cpl.name,cpl.email,cpl.mobile,cpl.fname,cpl.lname,cpl.current_ctc,\r\n" + 
					"exp.experience,sp.sourceof_profile,GROUP_CONCAT(ss.skillset_name),pl.preferedlocation_name,cpl.notice_period,cpl.current_location,cpl.education,cpl.current_designation,cpl.current_organization,cpl.previous_organization,\r\n" + 
					"DATEDIFF(CURDATE(),cpl.created_on) AS days FROM cpool_consultant cpl JOIN  cpool_preffered_location pl ON \r\n" + 
					"pl.prefered_location_id=cpl.fk_prefered_location_id JOIN cpool_experience exp ON \r\n" + 
					"exp.experience_id=cpl.fk_experience_id JOIN cpool_sourceof_profile sp ON \r\n" + 
					"sp.sourceof_profile_id=cpl.fk_sourceof_profile_id,cpool_skill_set ss WHERE   cpl.delete_status=0 \r\n" + 
					"AND FIND_IN_SET(ss.skillset_id,cpl.fk_skillset_id) AND cpl.fk_skillset_id \r\n" + 
					"LIKE '%"+fk_skillset_id+"%' AND cpl.fk_prefered_location_id LIKE '%"+fk_prefered_location_id+"%' AND \r\n" + 
    ...
    public List<ConsultantListResponseDTO> viewuserById(int consultant_id) {
            ...
            try {
             /*       String query = "SELECT cpl.consultant_id,"
                                    + " cpl.name,cpl.email,"
                                    + " cpl.mobile,"
                                    + " cpl.fname,"
                                    + " cpl.lname,"
                                    ...
                                    + " exp.experience,"
                                    + " sp.sourceof_profile,"
                                    + " GROUP_CONCAT(DISTINCT ss.skillset_name),"
                                    + " GROUP_CONCAT(DISTINCT pfd.preferedlocation_name),"
                                    + " pfd.preferedlocation_name,"
                                    + " cpl.current_location,"
                                    + " cpl.notice_period,"
                                    + " DATEDIFF(CURDATE(),cpl.created_on)"  
                                    + " AS days,"
                                    + " cpl.education,"
                                    + " cpl.previous_organization,"
                                    + " cpl.current_organization,"
                                    + " cpl.current_designation "
                                    + "FROM cpool_consultant cpl" +
                                    " JOIN cpool_sourceof_profile sp ON cpl.fk_sourceof_profile_id=sp.sourceof_profile_id JOIN" + 
                                    " cpool_experience exp ON cpl.fk_experience_id=exp.experience_id,"
                                    + "cpool_skill_set ss,cpool_preffered_location pfd where cpl.delete_status=0 AND cpl.consultant_id ='"+consultant_id
                                    +"' AND FIND_IN_SET(ss.skillset_id,cpl.fk_skillset_id) AND FIND_IN_SET(pfd.prefered_location_id,cpl.fk_prefered_location_id)"  
                                    +"  GROUP BY cpl.consultant_id;";*/
                    String query="SELECT cpl.consultant_id,cpl.name,cpl.email,cpl.mobile,cpl.fname,cpl.lname,cpl.current_ctc," + 
                    		"exp.experience,sp.sourceof_profile,GROUP_CONCAT(DISTINCT ss.skillset_name)," + 
                    		"GROUP_CONCAT(DISTINCT pfd.preferedlocation_name),cpl.current_location,cpl.notice_period,DATEDIFF(CURDATE()," +
                    		 "cpl.created_on) AS days,cpl.education,cpl.current_designation,cpl.current_organization,cpl.previous_organization " + 
                    		" FROM cpool_consultant cpl " + 
                    		"JOIN cpool_sourceof_profile sp ON cpl.fk_sourceof_profile_id=sp.sourceof_profile_id JOIN " + 
